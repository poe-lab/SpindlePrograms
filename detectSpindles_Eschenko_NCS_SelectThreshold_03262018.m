function detectSpindles_Eschenko_NCS_SelectThreshold_03262018
...
%% Determine threshold
threshMultiplierRMS = [];
while isempty(threshMultiplierRMS)
    ...
    threshMultiplierRMS = str2double(answer{1,1});
...
threshold = threshMultiplierRMS * stdDevNremRMS;
...
maxdist = 0.1;                                                              % Maximum distance between spindles to combine into same spindle
...
duration(duration<0.5 | duration>2) = NaN;
...
if exist(matFile, 'file') == 2
    save(matFile, 'EschenkoSpindle', 'TimeStamps', 'scoredFile', 'CSCFilename','threshMultiplierRMS', 'startTime', 'endTime', '-append');
else
    save(matFile, 'EschenkoSpindle', 'TimeStamps', 'scoredFile', 'CSCFilename','threshMultiplierRMS','startTime', 'endTime');
end
end